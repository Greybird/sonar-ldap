...
public class LdapSettingsManager implements ServerExtension {
  ...
  private ReverseProxySettings reverseProxySettings = null;
  ...
  private void initSimpleLdapConfiguration() {
    ...
    if (ldapUrl == null && realm != null) {
      ...
      if (ldapServers.isEmpty()) {
        throw MessageException.of(String.format("The property '%s' is empty and SonarQube is not able to auto-discover any LDAP server.", ldapUrlKey));
      }
      int index = 1;
      ...
      if (StringUtils.isBlank(ldapUrl)) {
        throw MessageException.of(String.format("The property '%s' is empty and no realm configured to try auto-discovery.", ldapUrlKey));
      }
  ...
  private void initMultiLdapConfiguration(String[] serverKeys) {
    if (settings.hasKey("ldap.url") || settings.hasKey("ldap.realm")) {
      throw MessageException.of("When defining multiple LDAP servers with the property '" + LDAP_SERVERS_PROPERTY + "', "
    ...
    for (String serverKey : serverKeys) {
      ...
      String ldapUrl = settings.getString(ldapUrlKey);
      if (StringUtils.isBlank(ldapUrl)) {
        throw MessageException.of(String.format("The property '%s' property is empty while it is mandatory.", ldapUrlKey));
      }
  ...
  public ReverseProxySettings getReverseProxySettings() {
    if (reverseProxySettings == null) {
      reverseProxySettings = new ReverseProxySettings(settings);
    }
